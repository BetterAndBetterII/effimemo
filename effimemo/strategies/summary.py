...
请提供压缩后的摘要："""
    ...
    def compress(self, messages: list, max_tokens: int, token_counter) -> list:
        ...
        # 保留最近的消息
        recent_messages = other_messages[-self.preserve_recent:] if len(other_messages) > self.preserve_recent else other_messages
        
        # 需要摘要的消息
        messages_to_summarize = other_messages[:-self.preserve_recent] if len(other_messages) > self.preserve_recent else []
