...
class TestSummaryCompressionStrategy:
    ...
    def test_simple_summary(self):
        ...
        messages = [
            {"role": "user", "content": "这是一个很长的用户消息，包含了很多重要的信息，需要被摘要处理"},
            {"role": "assistant", "content": "这是助手的回复，也包含了重要的信息"},
            {"role": "user", "content": "短消息"},  # 太短，应该被跳过
            {"role": "assistant", "content": "另一个很长的助手回复，包含了更多的详细信息和解释"}
        ]
    ...
    def test_create_summary_without_openai_client(self):
        ...
        messages = [
            ...
            {"role": "assistant", "content": "这是助手的详细回复，包含了重要的信息和解释"}
        ]
    ...
    def test_compress_no_compression_needed(self):
        ...
        # 假设这些消息的token数很少
        result = strategy.compress(messages, max_tokens=1000, token_counter=self.token_counter)
    ...
    def test_compress_with_summary(self):
        ...
        strategy = SummaryCompressionStrategy(
            openai_client=mock_client,
            preserve_recent=4  # 保留最近的4条消息（2轮对话）
        )
        ...
        result = strategy.compress(messages, max_tokens=max_tokens, token_counter=self.token_counter)
        
        # 检查结果结构
        system_messages = [m for m in result if m.get("role") == "system" and "[对话摘要]" not in m.get("content", "")]
        summary_messages = [m for m in result if "[对话摘要]" in m.get("content", "")]
        recent_messages = [m for m in result if m.get("role") in ["user", "assistant"] and "[对话摘要]" not in m.get("content", "")]
        ...
        if len(recent_messages) > 0:
            ...
            # 检查是否包含最新的消息
            assert any("最新" in content for content in recent_contents) or any("第三个" in content for content in recent_contents)
    ...
    def test_compress_preserve_system_false(self):
        ...
        result = strategy.compress(messages, max_tokens=30, token_counter=self.token_counter)
    ...
    def test_compress_only_system_messages(self):
        ...
        result = strategy.compress(messages, max_tokens=10, token_counter=self.token_counter)
    ...
    def test_compress_with_fallback_truncation(self):
        ...
        # 设置一个很小的token限制
        result = strategy.compress(messages, max_tokens=5, token_counter=self.token_counter)
